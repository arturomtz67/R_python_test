...
This is a cox proportional hazards model on data from [NHANES I](https://wwwn.cdc.gov/nchs/nhanes/nhanes1) with followup mortality data from the [NHANES I Epidemiologic Followup Study](https://wwwn.cdc.gov/nchs/nhanes/nhefs). It is designed to illustrate how SHAP values enable the interpretion of XGBoost models with a clarity traditionally only provided by linear models. 
...
```{python, fig.cap="The plot shows that mortality risk increases steadily with age, and the risk is amplified for individuals with higher systolic blood pressure.."}

shap.dependence_plot("age", shap_values, X_nhanes, interaction_index="systolic_blood_pressure")
```

```{python}
shap.dependence_plot("bmi", shap_values, X_nhanes, display_features=X_display, show=False)
plt.xlim(15, 55)
plt.show()
```
